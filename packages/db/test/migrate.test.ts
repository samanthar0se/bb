...
describe("migrate", () => {
  ...
  it("recreates thread plugin metadata after a populated pre-0116 rewind", () => {
    const db = createMigratedConnection();

    try {
      expect(migrationJournal.entries.at(-1)?.tag).toBe(
        "0116_smooth_masked_marvel",
      );
      const threadPluginMetadataMigrationWhen = migrationJournal.entries.find(
        (entry) => entry.tag === "0116_smooth_masked_marvel",
      )?.when;
      if (threadPluginMetadataMigrationWhen === undefined) {
        throw new Error("Missing 0116_smooth_masked_marvel migration timestamp");
      }
      const host = upsertHost(db, noopNotifier, {
